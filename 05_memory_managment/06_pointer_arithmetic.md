...
**Pointer** (указатель) — это переменная, содержащая **typed memory address** (типизированный адрес). Указатель сообщает компилятору, сколько данных находится по этому адресу и как их интерпретировать.
...
Оператор ` & ` (addres-of) возвращает указатель на переменную. Оператор ` * ` (dereferencing) обращается к значению, хранящемуся по адресу, на который указывает указатель.

В 64-битной системе указатель обычно занимает 8 байт, независимо от типа, на который он указывает. Типы ` int* `, ` double* ` и ` Connexion* ` имеют одинаковый размер, различается только их интерпретация.

**Basic pointer operators**

- ` & ` (address-of) - получить адрес объекта
- ` * ` (dereferencing) - разыменовать указатель

---

## Basic pointer arithmetic

При арифметических операциях, указатель **смещается на размер элемента на который указывает**. Другими словами, указатель сместится на ` sizeof(pointed_type) ` байт.
...
При использовании типов с большим размером, изменяется шаг смещения:
